...
daemon_user=radio
daemon_group=radio
supervize_dir=/etc/haloradio/svsdir-rpc
...
if [ ! -e ${daemon_dir}/HaloRadioRPC.sh ]; then
...
printf "#!/bin/sh\n\n# halo_radio RPC init script for daemontools\n\nexec setuidgid %s %s/HaloRadioRPC.sh 2>&1\n" ${daemon_user} ${daemon_dir} > run
...
echo "created supervise directory for halo_radio RPC at ${supervize_dir}."
